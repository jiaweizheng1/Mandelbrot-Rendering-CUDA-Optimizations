...
static int thread_count = 1; // default num_threads for OpenMP
static int n = 512;
static int m = 512;
static int dim = 1024;
...
void init_colours(void) {
    ...
    #pragma omp parallel for num_threads(thread_count) \
        default(none) shared(max_iter, colors, freq)
...
void usage(){
    ...
    printf("Usage: benchmark [p] [n] [m] [dim] [max_iter]\n");

    printf("\tp\t\t=\tnumber of OpenMP threads (defaults to 1)\n");
