...
__device__ uint32_t mandel_double(float cr, float ci, int max_iter) {
    float zr = 0;
    float zi = 0;
    float zrsqr = 0;
    float zisqr = 0;
    ...
    for (i = 0; i < max_iter; i++){
		...
		zi = __fadd_rd(zi, ci);
		//zr = zrsqr - zisqr + cr;
		zr = __fsub_rd(zrsqr, zisqr);
		zr = __fadd_rd(zr, cr);
		//zrsqr = zr * zr;
		zrsqr = __fmul_rd(zr, zr);
		//zisqr = zi * zi;
		zisqr = __fmul_rd(zi, zi);
		
    //the fewer iterations it takes to diverge, the farther from the set
		if (__fadd_rd(zrsqr, zisqr) > 4.0) break;
    }
...
__global__ void mandel_kernel(uint32_t *counts, float xmin, float ymin,
            float step, int max_iter, int dim, uint32_t *colors) {
    int pix_per_thread = dim * dim / (gridDim.x * blockDim.x);
    //int pix_per_thread = mulhi(dim, dim) / (gridDim.x * blockDim.x);
    int tId = blockDim.x * blockIdx.x + threadIdx.x;
    //int tId = mulhi(blockDim.x, blockIdx.x) + threadIdx.x;
    int offset = pix_per_thread * tId;
    //int offset = mulhi(pix_per_thread, tId);
    for (int i = offset; i < offset + pix_per_thread; i++){
        int x = i % dim;
        int y = i / dim;
        float cr = xmin + x * step;
	//float cr = __fmaf_rd(int2float_rd(x), step, xmin);
        float ci = ymin + y * step;
	//float ci = __fmaf_rd(int2float_rd(y), step, ymin);
    ...
    if (gridDim.x * blockDim.x * pix_per_thread < dim * dim
        ...
        int y = i / dim;
        float cr = xmin + x * step;
        //float cr = __fmaf_rd(__int2float_rd(x), step, xmin);
        float ci = ymin + y * step;
	//float ci = __fmaf_rd(__int2float_rd(y), step, ymin);
        counts[y * dim + x]  = colors[mandel_double(cr, ci, max_iter)];
    }
...
static void display_double(float xcen, float ycen, float scale,
        uint32_t *dev_counts, uint32_t *colors){
    dim3 numBlocks(dim,dim);
    float xmin = xcen - (scale/2);
    float ymin = ycen - (scale/2);
    float step = scale / dim;
